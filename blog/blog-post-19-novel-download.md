# 🔍 在 Electron 里造一个「搜书 + 下载」：从 so-novel 到 织梦书房 的爬虫实践
...
在 织梦书房 里，从首页侧栏进入「下载小说」，你会看到：顶部悬浮栏（返回首页 + 页面说明）、书源选择与搜索框、搜索结果列表，以及选中某本书后的目录信息与操作按钮。
...
织梦书房 是 Electron + Vue 3。**所有对外请求和 HTML 解析都放在主进程**，避免渲染进程直接请求第三方站点带来的跨域和安全性问题；渲染进程只通过 IPC 发「搜索 / 拉目录 / 下载章节」的指令，并接收进度事件。
...
在 Electron 里做「搜书 + 下载」，本质是：**主进程负责请求与解析，书源用配置驱动，渲染进程只发指令和展示**。借鉴 so-novel 的规则设计，用 Node + Cheerio + iconv-lite 实现多书源、GET/POST、编码与正文过滤，再把 IPC 和 Vue 响应式之间的「可序列化」问题处理掉，就能在 织梦书房 里实现一版可用的下载小说功能。
...
- **GitHub 仓库**：[织梦书房](https://github.com/zhimeng-shufang/zhimeng-shufang) — ⭐ **给个 Star 支持一下，让更多写作者发现这款工具！**
- **同系列**：书架管理、AI 封面、地图设计等可参考仓库内其他博客与源码。

### 🏷️ 标签

`#Electron` `#Vue3` `#爬虫` `#Cheerio` `#小说下载` `#so-novel` `#IPC` `#多书源` `#织梦书房` `#写作软件`

---

> 💡 **如果这篇文章对你有帮助，欢迎在 GitHub 给 织梦书房 点个 ⭐️ Star，让更多写作者和开发者发现这个项目！**
>
> 本文基于 织梦书房 开源项目「下载小说」功能，仅供学习与合规使用。
