...
> 💡 **完整前端实现**：选区获取、弹框左右布局、一键复制与确认替换等逻辑均在 [EditorPanel.vue](https://github.com/zhimeng-shufang/zhimeng-shufang/blob/main/src/renderer/src/components/Editor/EditorPanel.vue) 中，欢迎在 GitHub 仓库查看带注释的源码。

---

## 主进程与 DeepSeek：润色接口与 IPC

主进程里已有 DeepSeek 服务（如 `src/main/services/deepseek.js`），润色只新增一个**纯文本进、纯文本出**的接口即可。完整实现可在 [GitHub 仓库](https://github.com/zhimeng-shufang/zhimeng-shufang) 中按文件名搜索查看。
...
- ⭐ **Star 项目**：[GitHub 仓库](https://github.com/zhimeng-shufang/zhimeng-shufang)
...
- **AI 润色编辑器侧**：[EditorPanel.vue](https://github.com/zhimeng-shufang/zhimeng-shufang/blob/main/src/renderer/src/components/Editor/EditorPanel.vue)（按钮、下拉、弹框、选区/整章逻辑、一键复制与替换）
- **DeepSeek 润色服务**：[deepseek.js - polishChapter](https://github.com/zhimeng-shufang/zhimeng-shufang/blob/main/src/main/services/deepseek.js)（润色提示词与 Chat 调用）
- **主进程 IPC**：[index.js - deepseek:polish-text](https://github.com/zhimeng-shufang/zhimeng-shufang/blob/main/src/main/index.js)（润色 IPC 注册）
- **Preload 暴露**：[preload/index.js](https://github.com/zhimeng-shufang/zhimeng-shufang/blob/main/src/preload/index.js)（`polishTextWithAI` 暴露给渲染进程）
- **DeepSeek 官方文档**：[DeepSeek API Documentation](https://platform.deepseek.com/api-docs/)

### 🏷️ 标签

`#AI润色` `#DeepSeek` `#小说写作` `#织梦书房` `#Electron` `#Vue3` `#TipTap` `#编辑器` `#写作辅助` `#开源` `#创作效率`

---

> 💡 **如果这篇文章对你有帮助，请到 GitHub 给个 ⭐️ Star，就是对项目最好的支持！**
>
> 💡 **想直接看实现细节？欢迎在 [GitHub 仓库](https://github.com/zhimeng-shufang/zhimeng-shufang) 中打开上述代码文件，每个模块都有注释说明。**

_本文基于 51 码字（织梦书房）开源项目中的 AI 润色功能整理，技术栈：Electron、Vue 3、TipTap、DeepSeek API。_
