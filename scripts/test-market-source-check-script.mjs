...
assert.equal(source.includes('refreshMarketTrends'), true, 'script must call the real market refresh service')
...
assert.equal(/边关小卒|逆光短剧师|七猫惊雷榜/.test(source), false, 'script must not contain canned book rows')
assert.equal(/fakeFetch|response\(|Mock|mock/.test(source), false, 'script must not use fake source data')
