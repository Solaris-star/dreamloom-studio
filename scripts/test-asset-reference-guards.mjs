...
try {
  assert.deepEqual((await listAssets(path.join(root, '不存在目录'))).summary, {
    total: 0,
  ...
  const assets = (await listAssets(root)).items
  ...
  await assert.rejects(() => deleteAsset(root, characterAsset.id), /characters\.json.*imagePath/)
  await assert.rejects(() => deleteAsset(root, sceneAsset.id), /scenes\.json.*image/)
  await assert.rejects(() => deleteAsset(root, coverAsset.id), /mazi\.json.*coverUrl/)
  ...
  await assert.rejects(() => importAsset(root, {
        bookName: '测试书',
        type: 'character',
  ...
  const result = await deleteAsset(root, characterAsset.id)
  ...
  assert.equal((await listAssets(root, { type: 'trash' })).items.length, 1)
  assert.equal(
    (await listAssets(root, { includeTrash: true })).items.some((item) => item.status === 'trash'),
    true
  )

  fs.mkdirSync(path.dirname(imagePath), { recursive: true })
  fs.writeFileSync(imagePath, Buffer.from([0x89, 0x50, 0x4e, 0x47]))
  await assert.rejects(() => restoreAsset(root, result.item.id), /原位置已有同名文件/)
  fs.rmSync(imagePath)
  const restored = await restoreAsset(root, result.item.id)
  assert.equal(restored.success, true)
  assert.equal(restored.item.name, '林溪.png')
  assert.equal(fs.existsSync(imagePath), true)
  assert.equal((await listAssets(root, { type: 'trash' })).items.length, 0)
  ...
  const restoredAsset = (await listAssets(root)).items.find((item) => item.name === '林溪.png')
  const attached = await attachToBook(root, {
    id: restoredAsset.id,
    bookName: '第二本书',
    type: 'character'
  })
  ...
  const attachedAgain = await attachToBook(root, {
    id: restoredAsset.id,
    bookName: '第二本书',
    type: 'character'
  })
  ...
  const gif = await importAsset(root, {
    bookName: '第二本书',
    type: 'scene',
    fileName: '动图.gif',
  ...
  const jpeg = await importAsset(root, {
    bookName: '第二本书',
    type: 'character',
    fileName: '人物.jpg',
  ...
  const attachment = await importAsset(root, {
    bookName: '第二本书',
    fileName: '',
  ...
  assert.equal((await listAssets(root, { type: 'all', keyword: '不存在关键词' })).items.length, 0)
  const filtered = await listAssets(root, { bookName: '第二本书', keyword: '地图', type: 'map' })
  assert.equal(filtered.items.length, 1)
  assert.equal(filtered.summary.byType.map, 1)
  assert.equal((await listAssets(root, { bookName: '不存在' })).items.length, 0)
  assert.throws(() => getAssetFile(root, { id: 'invalid' }), /资产文件不存在|资产 ID 无效/)
  await assert.rejects(() => importAsset(root, { bookName: '不存在', fileName: '附件.txt', base64: 'dGVzdA==' }),
    /未找到目标书籍/
  )
  ...
  await assert.rejects(() => listAssets(root, { type: 'trash' }), /回收站记录读取失败/)
  fs.writeFileSync(path.join(root, 'assets-trash', 'manifest.json'), '{}', 'utf-8')
  await assert.rejects(() => listAssets(root, { type: 'trash' }), /回收站记录格式异常/)
