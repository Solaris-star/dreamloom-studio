...
const buildGraphPayload = () => ({
  ...
  nodes: clonePlainData(organizationData.value.nodes),
  lines: clonePlainData(organizationData.value.lines)
})

const addGraphItems = (node, line = null) => {
  const graphInstance = getGraphInstance()
  if (graphInstance?.addNodes) {
    graphInstance.addNodes([clonePlainData(node)])
    if (line && graphInstance.addLines) {
      graphInstance.addLines([clonePlainData(line)])
    }
    graphInstance.dataUpdated?.()
  } else {
    refreshGraph()
...
const updateNodeParent = () => {
  ...
  if (didRemovePreviousLine && graphInstance?.addLines) {
    graphInstance.addLines([clonePlainData(nextLine)])
  }
...
// 刷新图表数据
const refreshGraph = () => {
  graphRef.value.setJsonData(buildGraphPayload())
}
