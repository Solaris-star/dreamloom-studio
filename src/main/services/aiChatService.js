...
export async function sendChatMessage({ bookPath, messages, chapterContent, textProvider, systemPreset } = {}) {
  const systemParts = [systemPreset?.systemPrompt || DEFAULT_SYSTEM_PROMPT];

  if (bookPath) {
    try {
      const bookContext = await buildBookWritingContextBlock(bookPath, { outlineContent: chapterContent });
      if (bookContext) {
