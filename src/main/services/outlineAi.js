...
class OutlineAiService {
  async refineOutline(payload = {}, textAiService) {
  ...
  async splitOutline(payload = {}, textAiService) {
    const sourceContent = sanitizeText(payload.sourceContent)
    if (!sourceContent) {
  ...
  async runTask(payload = {}, textAiService) {
    const taskType = sanitizeText(payload.taskType)
    if (taskType === 'refine') {
      const result = await this.refineOutline(payload, textAiService)
      return {
        taskType,
        ...result
      }
    ...
    if (taskType === 'split') {
      const result = await this.splitOutline(payload, textAiService)
      return {
        taskType,
        ...result
      }
